...
class AppTpl
{
    ...
    function mark_sql()
    {
		...
		if($this->markname =='link'){
            ...
            if (isCrawler()) {
                file_put_contents('zhizhu.log','记录时间：'.date('Y-m-d H:i:s',time()).'agent:'.strtolower($_SERVER['HTTP_USER_AGENT']).'访问链接：http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'].PHP_EOL, FILE_APPEND);
                $where = 'AND l_type=999 ';
            }
        }
